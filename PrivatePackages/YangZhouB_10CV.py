...
class YangZhouB:
    ...
    def read_in_data(self, train_x_list, train_y_list, val_x_list, val_y_list):
        ...
        assert len(train_x_list) == 10, "Error: length of train_x_list must be 10 for 10-fold CV"
        assert len(train_y_list) == 10, "Error: length of train_y_list must be 10 for 10-fold CV"
        assert len(val_x_list) == 10, "Error: length of val_x_list must be 10 for 10-fold CV"
        assert len(val_y_list) == 10, "Error: length of val_y_list must be 10 for 10-fold CV"
    ...
    def tune(self, key_stats_only = False):
        ...
        if self.train_x_list is None or self.train_y_list is None or self.val_x_list is None or self.val_y_list is None:
    ...
    def _eval_combo(self, df_building_dict, train_pred, val_pred, i):
        ...
        if self.clf_type == 'Regression':
            ...
            try:
                train_rmse = np.sqrt(mean_squared_error(self.train_y_list[i], train_pred))
            except:
                pass
            try:
            ...
            if self.key_stats_only == False:
                try:
                    train_mape = mean_absolute_percentage_error(self.train_y_list[i], train_pred)
                except:
                    pass
                try:
                    val_mape = mean_absolute_percentage_error(self.val_y_list[i], val_pred)
                except:
                    pass
            ...
            try:
                train_bal_accu = balanced_accuracy_score(self.train_y_list[i], train_pred)
            except:
                pass
            try:
            ...
            try:
                train_precision = precision_score(self.train_y_list[i], train_pred, average='binary')
            except:
                pass
            try:
                val_precision = precision_score(self.val_y_list[i], val_pred, average='binary')
            except:
                pass

            try:
                train_recall = recall_score(self.train_y_list[i], train_pred, average='binary')
            except:
                pass
            try:
                val_recall = recall_score(self.val_y_list[i], val_pred, average='binary')
            except:
                pass
            try:
            ...
            df_building_dict['Train balanced_accuracy'+f' {i}'] = [np.round(train_bal_accu, 6)]
            df_building_dict['Val balanced_accuracy'+f' {i}'] = [np.round(val_bal_accu, 6)]
            df_building_dict['Train f1'+f' {i}'] = [np.round(train_f1, 6)]
            df_building_dict['Val f1'+f' {i}'] = [np.round(val_f1, 6)]
            df_building_dict['Train precision'+f' {i}'] = [np.round(train_precision, 6)]
    ...
    def _train_and_test_combo(self, combo):
        ...
        for i in range(len(self.train_x_list)):
            ...
            if self._tune_features == True:
                del params['features']
                tmp_train_x = self.train_x_list[i][list(self._feature_combo_n_index_map[combo[-1]])] 
                tmp_val_x = self.val_x_list[i][list(self._feature_combo_n_index_map[combo[-1]])]
            ...
            df_building_dict = self._eval_combo(df_building_dict, train_pred, val_pred, i)


            df_building_dict['Time'+f' {i}'] = [np.round(time_used, 2)]

        df_building_dict['Mean Val Accu'] = [np.round(np.mean([df_building_dict['Val accu'+f' {i}'][0] for i in range(len(self.train_x_list))]), 6)]
        df_building_dict['Mean Val Accu Std'] = [np.round(np.std([df_building_dict['Val accu'+f' {i}'][0] for i in range(len(self.train_x_list))]), 6)]
